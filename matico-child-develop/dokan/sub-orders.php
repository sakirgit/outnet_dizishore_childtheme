...
<table class="shop_table my_account_orders table table-striped shop_table_responsive">
    <thead>
        <tr>
    ...
    foreach ( $sub_orders as $order ) {
        ...
        ?>
            <tr class="order">
                <td class="order-number" data-title="<?php esc_attr_e( 'Order', 'dokan-lite' ); ?>">
                    <a href="<?php echo esc_url(  is_callable( [ $order, 'get_view_order_url' ] ) ? $order->get_view_order_url() : '#' ); ?>">
                        <?php echo esc_html( $order->get_order_number() ); ?>
                    </a>
                </td>
                <td class="order-date" data-title="<?php esc_attr_e( 'Date', 'dokan-lite' ); ?>">
                    <time datetime="<?php echo esc_attr( $order_date->format( 'Y-m-dTH:i:s' ) ); ?>">
                        <?php echo esc_html( dokan_format_date( $order_date ) ); ?>
                    </time>
                </td>
                <td class="order-status" style="text-align:left; white-space:nowrap;" data-title="<?php esc_html_e( 'Status', 'dokan-lite' ); ?>">
                ...
                <?php endif; ?>
                <td class="order-total" data-title="<?php esc_attr_e( 'Total', 'dokan-lite' ); ?>">
                    <?php
                    echo wp_kses_post(
                        sprintf(
                ...
                <td class="order-actions" data-title="<?php esc_attr_e( 'Action', 'dokan-lite' ); ?>">
                    <?php
                        $actions = array();
