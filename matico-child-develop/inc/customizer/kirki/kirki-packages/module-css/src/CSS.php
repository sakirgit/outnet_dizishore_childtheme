...
class CSS {
	...
	public function field_init( $args, $object ) {
		...
		if ( ! is_array( $args['output'] ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"output" invalid format in field %s. The "output" argument should be defined as an array of arrays.', 'matico-child' ), esc_html( $args['settings'] ) ), '3.0.10' );
			$args['output'] = array(
				array(
					'element' => $args['output'],
				),
